import type { SlotConfig, WinLine } from "@/types/SlotConfig"
...
export function validateSlotConfig(config: unknown): SlotConfigValidationResult {
    ...
    const value = config as Partial<SlotConfig>
...
function validateAvailableSymbolType(value: unknown, field: string, errors: string[]): void {
    if (typeof value !== 'string' || !["defaults", "fruits", "egyptians", "sports"].includes(value)) {
        errors.push(`${field} do not accept that ${value}.`)
    }
}

function validateAvailableSoundType(value: unknown, field: string, errors: string[]): void {
    if (typeof value !== 'string' || !["defaults", "fruits", "cyberpunk", "fantasy"].includes(value)) {
        errors.push(`${field} do not accept that ${value}.`)
    }
}
